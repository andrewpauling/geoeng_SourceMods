...
!ocl novrec      !Fujitsu
    ...
    !-------------------------------------------------------------------
    ! AGP add hosing of ice surface, modified from freeboard function
    !-------------------------------------------------------------------

      call floodice (nx_block, ny_block, &
                      icells,      &
                      indxi,    indxj,    &
                      dt,                 &
                      snoice,             &
...
!BOP
!
! !ROUTINE: floodice - hosing of ice surface
!
! !DESCRIPTION:
!
! Calculate enthalpy change due to flooding surface of ice with seawater
! and converting snow to ice
! 
!
! !REVISION HISTORY:
!
...
! !INTERFACE:
!
      subroutine floodice (nx_block, ny_block, &
                            icells,      &
                            indxi,    indxj,    &
                            dt,                 &
                            snoice,             &
...
!
      ...
      do ij = 1, icells
         ...
         dhsn(ij) = c0
         hqs (ij) = c0
         HOSE (ij) = 1.2/(365.0*24.0*2) ! AGP amount to hose snow surface

         ! AGP height change of snow exactly equal to height change of ice
         ! AGP hose only in winter in northern hemisphere
         if (lmask_n(i,j)) then
            if (month > 8 .and. month < 11) then
               if (HOSE(ij) > puny .and. hsn(ij) > puny) then  ! snow below freeboard
                  dhsn(ij) = min(HOSE(ij), hsn(ij))            ! snow to remove
                  dhin(ij) = dhsn(ij)        ! ice to add
               endif
            endif
         endif
      enddo
